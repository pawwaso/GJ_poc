...
28. `create or replace stream account_states_stream as select REGEXP_EXTRACT('^(.*)=(.*)}$', partner_id,2) as PARTNER_ID, iban, parse_timestamp(gueltig_ab,'dd.MM.yyyy') as gueltig_ab, cast(kontostand as double) as kontostand from ACCOUNT_STATES_STREAM_KEYED partition by REGEXP_EXTRACT('^(.*)=(.*)}$', partner_id,2)  emit changes;`    
